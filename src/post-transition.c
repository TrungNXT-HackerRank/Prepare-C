...
// int total_weight(post_office p)
// {
// 	int sum = 0;
// 	for (int i = 0; i < p.packages_count; p++)
// 	{
// 		sum += p.packages[i].weight;
// 	}
// 	return sum;
// }
...
{
	// if (total_weight(source->offices[source_office_index]) + total_weight(target->offices[target_office_index] <= target->offices[target_office_index].max_weight))
	// {
	// 	Package *newPackages = malloc(sizeof(package) * (source->offices[source_office_index].packages_count));
	// 	target->offices[target_office_index].packages_count += source->offices[source_office_index].packages_count;
	// 	for (i = 0; i < source->offices[source_office_index]; i++)
	// 	{
	// 		newPackages[i] = source->offices[source_office_index].packages[i];
	// 		target->offices[source_office_index].packages[target->offices[target_office_index].packages_count + i] = newPackages[i];
	// 		free(source->offices[source_office_index].packages[i]);
	// 	}
	// 	source->offices[source_office_index].packages_count = 0;
	// }
	int n = 0;
...
{
	int ans = 0;
	int max_packages = total_packages(towns[0]);

	for (int i = 1; i < towns_count; i++)
	{
		if (total_packages(towns[i]) > max_packages)
		{
			max_packages = total_packages(towns[i]);
			ans = i;
		}
	}
